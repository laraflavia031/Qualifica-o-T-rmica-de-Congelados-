...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        tipo_label = {
            "refrigerador": "❄️ Refrigerador / Câmara Fria",
            "galpao": "🏭 Galpão",
            "freezer": "🧊 Freezer",
            "container": "📦 Container",
        }.get(st.session_state.tipo_equipamento, st.session_state.tipo_equipamento)
        modo_label = ""
        if st.session_state.tipo_equipamento in ["freezer", "container"]:
            modo_label = f" — {'Maturação' if st.session_state.modo_equipamento == 'maturacao' else 'Conservação'}"
        st.markdown(f"**Tipo:** {tipo_label}{modo_label}")
    with col2:
        ...
        try:
            ...
            for estudo_info in gerador.estudos_info:
                try:
                    ...
                    raise RuntimeError(f"Data/hora inválida no estudo '{estudo_info['nome']}': {e}")

                # Para estudos de maturação, a duração é calculada automaticamente
                # — buscar em todos os dados a partir do início
                eh_maturacao = estudo_info.get('teste_maturacao', False)

                arquivos_com_dados = []
                for arq_idx, df_arq in gerador.dados_por_arquivo.items():
                    if eh_maturacao:
                        # Buscar dados a partir do início sem limite de fim
                    ...
                    if len(df_filtrado) > 0:
                        ...
                        # Para maturação, calcular duração real e atualizar estudo_info
                        if eh_maturacao:
                            ts_max = df_filtrado['timestamp'].max()
                            duracao_real = (ts_max - data_inicio_dt).total_seconds() / 3600
                            # Adicionar 24h para a estabilização automática
                            estudo_info['duracao_horas'] = duracao_real
                            estudo_info['duracao_minutos'] = duracao_real * 60
                            log_ui(f"  ℹ️ Maturação '{estudo_info['nome']}': duração real = {duracao_real:.1f}h")

                if len(arquivos_com_dados) == 0:
                    periodo_txt = f"a partir de {data_inicio_dt.strftime('%d/%m/%Y %H:%M')}"
                    if not eh_maturacao:
                        data_fim_dt = data_inicio_dt + timedelta(hours=estudo_info['duracao_horas'])
                        periodo_txt = f"{data_inicio_dt.strftime('%d/%m/%Y %H:%M')} a {data_fim_dt.strftime('%d/%m/%Y %H:%M')}"
                    raise RuntimeError(
                        f"Nenhum arquivo contém dados para o estudo '{estudo_info['nome']}' "
                        f"no período {periodo_txt}."
                    )
            ...
            for estudo_info in gerador.estudos_info:
                ...
                # Umidade: não gerar para estudos de maturação
                eh_maturacao = estudo_info.get('teste_maturacao', False)
                if gerar_umidade and tem_umidade and not eh_maturacao and gerador.sensores_por_arquivo[arquivo_idx]['umidade']:
