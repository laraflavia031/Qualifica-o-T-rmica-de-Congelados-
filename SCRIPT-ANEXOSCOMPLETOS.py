...
def calcular_tempo_maturacao(df, sensores_dict, sensor_externo, limite_min, limite_max):
    """
    Calcula o tempo de maturação permitindo oscilação de até 12 horas.
    
    Lógica:
    1. Encontra o ponto onde TODOS os sensores entram no critério
    2. Permite oscilação de até 12 horas
    3. Depois que passa por esse período, conta como maturado
    ...
    idx_primeiro_ok = indices_ok[0]
    
    # Procurar por um período de 12 horas onde a maioria fica no critério
    # Permitir oscilação de até 12 horas (720 minutos)
    # Assumindo dados em intervalos de 1 minuto, 720 linhas = 720 minutos = 12 horas
    janela_minutos = 720
    
    # Encontrar a primeira janela onde pelo menos 95% dos pontos estão no critério
    idx_maturacao = idx_primeiro_ok
    
    for i in range(idx_primeiro_ok, len(df_verificacao) - janela_minutos):
        janela = df_verificacao.loc[i:i+janela_minutos-1, 'todos_ok']
        percentual_ok = (janela.sum() / len(janela)) * 100
        
        # Se 95% ou mais da janela está ok, consideramos como maturado
        if percentual_ok >= 95:
            idx_maturacao = i
            break
...
class GeradorRelatorioGxP:
    ...
    def calcular_maturacao_para_estudo(self, idx_estudo, estudo, df_estudo, arquivo_idx):
        ...
        # Verificar se este estudo específico é de maturação
